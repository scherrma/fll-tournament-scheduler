...
def read_data(fpath):
    ...
    try:
        ...
        j_duration = timedelta(minutes=param["j_duration"])
        j_duration_team = timedelta(minutes=10)
        j_consec = param["j_consec"]
        j_break = timedelta(minutes=param["j_break"])
        j_lunch = datetime.combine(datetime(1, 1, 1), param["j_lunch"])
        j_lunch_duration = timedelta(minutes=param["j_lunch_duration"])
        ...
        t_lunch_duration = timedelta(minutes=param["t_lunch_duration"])
    ...
    return ((teams, divisions, scheduling_method, travel, coach_meet, opening, j_start, j_sets,
             j_calib, j_duration, j_duration_team, j_consec, j_break, j_lunch, j_lunch_duration,
             t_rounds, t_pairs, t_duration, t_lunch, t_lunch_duration),
            (tournament_name, event_names, rooms))

def export(tment, fpath, tournament_name, event_names, rooms):
    ...
    team_width = len(tment.teams[0].info(tment.divisions))

    team_sheet = pandas.read_excel(fpath, sheet_name="Team Information")
    column_check = ["Team Number", "Team"] + tment.divisions*["Division"]
    ...
    export_judge_views(tment, workbook, time_fmt, team_width, team_info, event_names, rooms)
    export_table_views(tment, workbook, time_fmt, team_width, team_info, event_names, rooms)
    ...
    while not saved:
        try:
            final_fout = outfpath.format(' ({})'.format(count) if count else '')
...
def export_judge_views(tment, workbook, time_fmt, team_width, team_info, event_names, rooms):
    ...
    for sheet in [ws_overall] + cat_sheets: #formatting
        ...
        util.ws_borders(sheet, ((styles.Border(left=thick), team_width*tment.j_sets, 2, 0),
...
def export_table_views(tment, workbook, time_fmt, team_width, team_info, event_names, rooms):
...
def generate_schedule():
    ...
    try:
        logic_params, io_params = read_data(fpath)
        tment = Tournament(*logic_params)
        tment.schedule()
        print("Exporting schedule")
        export(tment, fpath, *io_params)
